  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Ringkasan keuangan kamu hari ini</p>
      </div>
      ...
      {/* Summary bulan ini */}
      <div>
        <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-3">{namaBulan}</p>
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-green-50 rounded-xl p-4">
            ...
            <p className="text-lg font-semibold text-green-700">{formatRupiah(pemasukanBulanIni)}</p>
          </div>
          <div className="bg-red-50 rounded-xl p-4">
            ...
            <p className="text-lg font-semibold text-red-600">{formatRupiah(pengeluaranBulanIni)}</p>
          </div>
        </div>
      </div>
      ...
      {(pemasukanBulanIni > 0 || pengeluaranBulanIni > 0) && (
        <div className={`rounded-xl p-4 flex items-center justify-between
          ${pemasukanBulanIni >= pengeluaranBulanIni ? "bg-green-50" : "bg-red-50"}`}>
          <p className={`text-sm font-medium ${pemasukanBulanIni >= pengeluaranBulanIni ? "text-green-700" : "text-red-600"}`}>
            {pemasukanBulanIni >= pengeluaranBulanIni ? "Surplus bulan ini" : "Defisit bulan ini"}
          </p>
          <p className={`text-base font-semibold ${pemasukanBulanIni >= pengeluaranBulanIni ? "text-green-700" : "text-red-600"}`}>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-semibold text-gray-700">Transaksi terbaru</p>
          <a href="/transaksi" className="text-xs text-purple-700 hover:underline">Lihat semua</a>
        </div>
        ...
        {transaksiTerbaru.length === 0 ? (
          ...
          <div className="space-y-2">
            {transaksiTerbaru.map((t) => (
              <div key={t.id} className="bg-white rounded-xl border border-gray-100 p-3.5 flex items-center justify-between">
